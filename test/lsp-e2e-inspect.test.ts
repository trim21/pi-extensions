// 端到端回归测试：真实 typescript-language-server（tsserver）下的只读 LSP 符号查询工具
...
async function setupProject(): Promise<{ directory: string; cleanup: () => Promise<void> }> {
  ...
  // fixture 的 lsp.json 是静态文件：cp 后在临时副本注入仓库 tsserver.path
  const lspConfigPath = join(directory, ".pi", "lsp.json");
  const lspConfig = JSON.parse(await readFile(lspConfigPath, "utf8")) as {
    servers: Record<string, { initializationOptions?: Record<string, unknown> }>;
  };
  lspConfig.servers.typescript.initializationOptions = { tsserver: { path: REPO_TSSERVER } };
  await writeFile(lspConfigPath, JSON.stringify(lspConfig, null, 2));
  return {
    directory,
...
/**
 * 只读查询没有 rename 那样的收敛校验：typescript-language-server 索引未加载完时会给出过时结果

...
- Commands run through the bwrap sandbox (two independent axes: fs `readonly` / `workspace-write` / `allow-all` and network `block` / `limited` / `allow-all`, switchable via `/bwrap-fs-*` and `/bwrap-network-*` commands). `dangerouslyDisableSandbox: true` requests one-time unsandboxed execution. Approval flow: commands are parsed (tree-sitter, including nested `$(...)`) and matched against `approvalRules` from `sandbox.json` — an `allow` rule auto-approves, a `deny` rule rejects outright (last matching rule wins), and only unmatched commands show the approval dialog. In headless sessions unsandboxed execution is denied.
